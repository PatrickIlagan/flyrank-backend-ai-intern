# Task Management Database API (SQLite)

A persistent CRUD REST API built with **Python 3.11** and **FastAPI**, backed by a local **SQLite** database (tasks.db). This project replaces Week 2 in-memory storage with disk-persisted relational data while preserving identical API contracts and status codes.
...
cd "flyrank-backend-ai-intern/Week 3/backend/BE_W3A2"
`
...
| Method | Endpoint | SQL Query / Action | Success Status | Error Status Codes |
| :--- | :--- | :--- | :--- | :--- |
| GET | / | API root descriptor metadata | 200 OK | -- |
| GET | /health | Server health check for uptime monitoring probes | 200 OK | -- |
| GET | /stats | SELECT COUNT(*) FROM tasks (computed total, done, open) | 200 OK | -- |
...
| POST | /reset | Restore starter seed tasks | 200 OK | -- |
